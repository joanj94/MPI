...
int convolve2DMPI(int* in, int* out, int len, char* type){
    ...
    #pragma omp parallel num_threads(4)
    {
        ...
        int dataSizeX = dataSizeXMPI;
        int dataSizeY = dataSizeYMPI/size;
        float *kPtr;
        int rowMin, rowMax;                             // to check boundary of input array
        int colMin, colMax;                             //
        float sum;                                      // temp accumulation buffer

        int tid = omp_get_thread_num(); //ID del fil
        int tChunk = dataSizeY/omp_get_num_threads(); //Block per a cada fil
        int tStart = tChunk * tid; //Inici del fil
        int tEnd = (tid == omp_get_num_threads()-1) ? tEnd = dataSizeY : tChunk * (tid+1); //El final del fil serà el final del serie si és l'últim o sinó fins l'inici del següent fil 

        kPtr = kern->vkern;
        inPtr = inPtr2 = &in[dataSizeX * kCenterY + kCenterX];  // note that  it is shifted (kCenterX, kCenterY),
        outPtr = out;

        int *pOutPtr = outPtr + (tStart*dataSizeX);
        int *tInPtr = inPtr + (tStart*dataSizeX);
        int *tInPtr2 = tInPtr;

        //printf("[%s] Rank: %d tid:%d tStart: %d tEnd: %d\n",type,rank,tid,tStart,tEnd);

        for(i= tStart; i < tEnd; ++i)                   // number of rows
        {
            // compute the range of convolution, the current row of kernel should be between these
            rowMax = i + kCenterY;
            rowMin = i - dataSizeY + kCenterY;
            //printf("[%s] Rank: %d tid:%d tStart: %d i(%d) tEnd: %d\n",type, rank,tid,tStart,i,tEnd);
...
int convolve2DMPI2(int* in, int* out, int len){
	int kCenterX, kCenterY;
...
void processFrontiers(int row, int kCenter){
	...
	convolve2DMPI2(tmpRBuffIn, tmpRBuffOut, (int)kern->kernelX); 
	convolve2DMPI2(tmpGBuffIn, tmpGBuffOut, (int)kern->kernelX); 
	convolve2DMPI2(tmpBBuffIn, tmpBBuffOut, (int)kern->kernelX); 
...
int main(int argc, char **argv)
{
    ...
    while (c < partitions) {
		...
		convolve2DMPI(Inbuff, Outbuff, dataSizeYMPI/size, "R");
		MPI_Barrier(MPI_COMM_WORLD);
		...
		convolve2DMPI(Inbuff, Outbuff, dataSizeYMPI/size, "G");
		MPI_Barrier(MPI_COMM_WORLD);
		...
		convolve2DMPI(Inbuff, Outbuff, dataSizeYMPI/size, "B");
		MPI_Barrier(MPI_COMM_WORLD);
		...
		if(rank == 0){
        	...
        	#pragma paraller num_threads(4) for
